...
async function buildState(keys, selectedHash, trackedLimitId) {
    ...
    if (hash) {
        ...
        // Meta
        try {
            meta = await api.fetchMeta();
        }
        catch { /* meta not available */ }
    }
    // Persist tracked limit
    const effectiveTracked = trackedLimitId ?? budgetLimits[0]?.id ?? null;
    // Update status bar
    const tracked = budgetLimits.find(b => b.id === effectiveTracked);
    if (tracked) {
        (0, status_bar_1.updateStatusBar)(tracked.used, tracked.limitUsd, tracked.name);
    }
    else {
...
async function showDashboard(context) {
    ...
    currentPanel = vscode.window.createWebviewPanel('openrouterTrackerDashboard', 'OpenRouter Tracker', vscode.ViewColumn.One, { enableScripts: true, retainContextWhenHidden: true });
    ...
    currentPanel.webview.onDidReceiveMessage(async (msg) => {
        try {
            if (msg.type === 'refresh') {
                ...
                const storedHash = context.globalState.get('selectedKeyHash') ?? null;
                const state = await buildState(keys, storedHash, context.globalState.get('trackedLimitId') ?? null);
                ...
                const keys = await api.listKeys();
                const state = await buildState(keys, msg.hash, context.globalState.get('trackedLimitId') ?? null);
                currentPanel?.webview.postMessage({ type: 'state', state });
            }
            else if (msg.type === 'setTrackedLimit') {
                await context.globalState.update('trackedLimitId', msg.limitId);
                const keys = await api.listKeys();
                const state = await buildState(keys, context.globalState.get('selectedKeyHash') ?? null, msg.limitId);
                currentPanel?.webview.postMessage({ type: 'state', state });
            }
...
function defaultTimeRange(gran) {
    const now = new Date();
